...
def extract_info():
	...
	for i in xrange(len(DATA_LIST)):
		...
		data = DATA_LIST[i]
		pattern = re.compile(r'AFF\s*NO\s*:\s*(\w+)\s*(\(\s*EXAM\s*NO\s*:\s*(\w+)\))?\s*THE\s*PRINCIPAL\s*([\s\S]+?)Pin:(\d+)?')
...
class SchoolRow:
	...
	def save(self):
		...
		curs.execute("INSERT INTO school_info (aff_num, exam_num, name, address, pin, cluster) VALUES (%s, %s, %s, %s, %s, %s)",
