...
from core.view_models.submission_id import CorrectedSubmissionBody, UncorrectedSubmissionBody
...
class SubmissionIdGetCorrected(SubmissionIdDriver):
    ...
    def student_endpoint(self):
        ...
        return render(self.request, self.template,
                      AuthenticatedBase(StudentSidebar(self.user), CorrectedSubmissionBody(self.submission))
                      .as_context())
    ...
    def parent_endpoint(self):
        if not self.parent_valid():
            return HttpResponseNotFound()
        return render(self.request, self.template,
                      AuthenticatedBase(ParentSidebar(self.user), CorrectedSubmissionBody(self.submission)))

    def admin_endpoint(self):
        if not self.admin_valid():
            return HttpResponseNotFound()
        return render(self.request, self.template,
                      AuthenticatedBase(AdminSidebar(self.user), CorrectedSubmissionBody(self.submission)))

    def teacher_endpoint(self):
        if not self.teacher_valid():
            return HttpResponseNotFound()
        return render(self.request, self.template,
                      AuthenticatedBase(TeacherSidebar(self.user), CorrectedSubmissionBody(self.submission)))
...
class SubmissionIdPostUncorrected(SubmissionIdUncorrected):
    def student_endpoint(self):
        ...
        # TODO: submission saved successfully message (also for announcement and other forms)
        return render(self.request, self.template,
                      AuthenticatedBase(StudentSidebar(self.user),
                                        UncorrectedSubmissionBody(self.submission, submission_form)).as_context())
