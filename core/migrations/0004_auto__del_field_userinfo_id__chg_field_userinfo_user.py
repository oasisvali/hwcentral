...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Deleting field 'UserInfo.id'
        db.delete_column(u'core_userinfo', u'id')


        # Changing field 'UserInfo.user'
        db.alter_column(u'core_userinfo', 'user_id',
                        self.gf('django.db.models.fields.related.OneToOneField')(to=orm['auth.User'], unique=True,
                                                                                 primary_key=True))

    def backwards(self, orm):
        # User chose to not deal with backwards NULL issues for 'UserInfo.id'
        raise RuntimeError("Cannot reverse this migration. 'UserInfo.id' and its values cannot be restored.")

        # The following code is provided here to aid in writing a correct migration        # Adding field 'UserInfo.id'
        db.add_column(u'core_userinfo', u'id',
