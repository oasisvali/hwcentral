...
MathJax.Extension["TeX/color"] = {
    version: "2.5.0",
    config: MathJax.Hub.CombineConfig("TeX.color", {padding: "5px", border: "2px"}),
    colors: {
        Apricot: "#FBB982",
        Aquamarine: "#00B5BE",
        Bittersweet: "#C04F17",
    ...
    getColor: function (a, c) {
        ...
        var b = this["get_" + a];
        if (!b) {
            this.TEX.Error(["UndefinedColorModel", "Color model '%1' not defined", a])
        }
        return b.call(this, c)
    },
    get_rgb: function (b) {
        b = b.replace(/^\s+/, "").replace(/\s+$/, "").split(/\s*,\s*/);
        var a = "#";
        if (b.length !== 3) {
        ...
        for (var c = 0; c < 3; c++) {
            ...
            if (d < 0 || d > 1) {
                this.TEX.Error(["ModelArg2", "Color values for the %1 model must be between %2 and %3", "rgb", 0, 1])
            }
    ...
    get_RGB: function (b) {
        ...
        for (var c = 0; c < 3; c++) {
            ...
            var d = parseInt(b[c]);
            if (d > 255) {
                this.TEX.Error(["ModelArg2", "Color values for the %1 model must be between %2 and %3", "RGB", 0, 255])
            }
            d = d.toString(16);
            if (d.length < 2) {
                d = "0" + d
    ...
    get_gray: function (a) {
        ...
        var b = parseFloat(a);
        if (b < 0 || b > 1) {
            this.TEX.Error(["ModelArg2", "Color values for the %1 model must be between %2 and %3", "gray", 0, 1])
        }
...
MathJax.Hub.Register.StartupHook("TeX Jax Ready", function () {
    var d = MathJax.InputJax.TeX, a = MathJax.ElementJax.mml;
    var c = d.Stack.Item;
    ...
    d.Parse.Augment({
        Color: function (h) {
            var g = this.GetBrackets(h), e = this.GetArgument(h);
            e = b.getColor(g, e);
            var f = c.style().With({styles: {mathcolor: e}});
            this.stack.env.color = e;
            this.Push(f)
        }, TextColor: function (h) {
            var g = this.GetBrackets(h), f = this.GetArgument(h);
            f = b.getColor(g, f);
            ...
            var f = this.GetArgument(g), e = this.GetArgument(g), h = this.GetArgument(g);
            b.colors[f] = b.getColor(e, h)
        }, ColorBox: function (g) {
            var f = this.GetArgument(g), e = this.InternalMath(this.GetArgument(g));
            this.Push(a.mpadded.apply(a, e).With({mathbackground: b.getColor("named", f)}).With(b.padding()))
        }, fColorBox: function (g) {
            var h = this.GetArgument(g), f = this.GetArgument(g), e = this.InternalMath(this.GetArgument(g));
            this.Push(a.mpadded.apply(a, e).With({
                mathbackground: b.getColor("named", f),
                style: "border: " + b.config.border + " solid " + b.getColor("named", h)
            }).With(b.padding()))
        }
    });
